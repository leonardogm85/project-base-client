...
import { faPlus, faTable, faSearch, faPencilAlt, faCheck, faBan, faPaperPlane, faList, faBars } from '@fortawesome/free-solid-svg-icons';
...
export class ListarComponent implements OnInit, OnDestroy {
  ...
  iconSendEmailConfirmationToken: IconDefinition = faPaperPlane;
  ...
  get tableResult(): TableResult<UserTable> {
    return this._tableService.result;
  }

  constructor(
    private _usuarioService: UsuarioService,
    private _tableService: TableService<UserTable>,
  ...
  ngOnInit(): void {
    this.subscription.add(this._tableService.onTable(f => this._usuarioService.obterTabela(f)).subscribe(r => this._tableService.setResult(r)));
  }
  ngOnDestroy(): void {
  ...
  onActivate(id: string): void {
    this.confirm(() => this._usuarioService.ativar(id), ValidationMessages.confirmarAtivacaoRegistro, ValidationMessages.registroAtivado);
  }
  onDeactivate(id: string): void {
    this.confirm(() => this._usuarioService.desativar(id), ValidationMessages.confirmarDesativacaoRegistro, ValidationMessages.registroDesativado);
  }
  onSendEmailConfirmationToken(id: string): void {
    this.confirm(() => this._usuarioService.enviarTokenConfirmacaoPorEmail(id),
      ValidationMessages.confirmarUsuarioEnvioEmailConfirmacao, ValidationMessages.registroUsuarioEnvioEmailConfirmacao);
  }
