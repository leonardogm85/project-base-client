...
export class AdicionarAutorizacoesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this._usuarioService.obterFormularioAutorizacao();
    this.patchValue();
  }

  patchValue(): void {
    this._usuarioService.obterAutorizacoes(this._route.snapshot.paramMap.get('id')).subscribe(p => {
      this.form.patchValue({ id: p.id, concurrencyStamp: p.concurrencyStamp, name: p.name });
      const menus = (this.form.get('menus') as FormArray);
      menus.clear();

      p.menus.forEach(m => {
        const menu = this._usuarioService.obterFormularioMenu();
        const items = (menu.get('items') as FormArray);
        items.clear();
        let menuEnabled = true;

        m.items.forEach(i => {
          const item = this._usuarioService.obterFormularioItem();
          const accesses = (item.get('accesses') as FormArray);
          accesses.clear();
          let itemEnabled = true;

          i.accesses.forEach(a => {
            const access = this._usuarioService.obterFormularioAcesso();
  ...
  onSave(): void {
    ...
    this._usuarioService.adicionarAutorizacoes(this.form.value).pipe(
