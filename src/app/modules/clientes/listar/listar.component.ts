...
import { ClienteTable } from 'src/app/models/cliente-table';
...
export class ListarComponent implements OnInit, OnDestroy {
  ...
  get tableResult(): TableResult<ClienteTable> {
    return this._tableService.result;
  }

  constructor(
    private _clienteService: ClienteService,
    private _tableService: TableService<ClienteTable>,
  ...
  ngOnInit(): void {
    this.subscription.add(this._tableService.onTable(f => this._clienteService.obterTabela(f)).subscribe(r => this._tableService.setResult(r)));
  }
  ngOnDestroy(): void {
  ...
  onActivate(id: string): void {
    this.confirm(() => this._clienteService.ativar(id), ValidationMessages.confirmarAtivacaoRegistro, ValidationMessages.registroAtivado);
  }
  onDeactivate(id: string): void {
    this.confirm(() => this._clienteService.desativar(id), ValidationMessages.confirmarDesativacaoRegistro, ValidationMessages.registroDesativado);
  }
  onDelete(id: string): void {
    this.confirm(() => this._clienteService.remover(id), ValidationMessages.confirmarRemocaoRegistro, ValidationMessages.registroRemovido);
  }
