...
import { faPlus, faTable, faSearch, faPencilAlt, faCheck, faBan, faList } from '@fortawesome/free-solid-svg-icons';
import { faTrashAlt } from '@fortawesome/free-regular-svg-icons';
import { NgxSpinnerService } from 'ngx-spinner';

import { RoleTable } from 'src/app/models/role-table';
...
export class ListarComponent implements OnInit, OnDestroy {
  ...
  get tableResult(): TableResult<RoleTable> {
    return this._tableService.result;
  }

  constructor(
    private _papelService: PapelService,
    private _tableService: TableService<RoleTable>,
  ...
  ngOnInit(): void {
    this.subscription.add(this._tableService.onTable(f => this._papelService.obterTabela(f)).subscribe(r => this._tableService.setResult(r)));
  }
  ngOnDestroy(): void {
  ...
  onActivate(id: string): void {
    this.confirm(() => this._papelService.ativar(id), ValidationMessages.confirmarAtivacaoRegistro, ValidationMessages.registroAtivado);
  }
  onDeactivate(id: string): void {
    this.confirm(() => this._papelService.desativar(id), ValidationMessages.confirmarDesativacaoRegistro, ValidationMessages.registroDesativado);
  }
  onDelete(id: string): void {
    this.confirm(() => this._papelService.remover(id), ValidationMessages.confirmarRemocaoRegistro, ValidationMessages.registroRemovido);
  }
