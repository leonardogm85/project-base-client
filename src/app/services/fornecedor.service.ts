...
import { FornecedorTable } from '../models/fornecedor-table';
...
export class FornecedorService {
  ...
  private readonly _api: string = `${environment.apiBase}/fornecedores`;
  ...
  obter(id: string): Observable<Fornecedor> {
    return this._http.get<Fornecedor>(`${this._api}/obter/${id}`).pipe(take(1));
  }

  adicionar(model: Fornecedor): Observable<NotificationResult> {
    return this._http.post<NotificationResult>(`${this._api}/adicionar`, model).pipe(take(1));
  }

  atualizar(model: Fornecedor): Observable<NotificationResult> {
  ...
  obterTabela(tableFilter: TableFilter): Observable<TableResult<FornecedorTable>> {
    return this._http.post<TableResult<FornecedorTable>>(`${this._api}/obter-tabela`, tableFilter).pipe(take(1));
  }
